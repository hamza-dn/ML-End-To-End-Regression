...
Flow:
1) Load train/eval final datasets.
2) Try multiple XGBoost parameter combinations with Optuna.
3) Track each trial in MLflow.
4) Retrain the best configuration and save the best model.
"""
...
DEFAULT_TRAIN = Path("data/processed/train_final.csv")
...
def _maybe_sample(
    ...
    """Optionally subsample data for faster tuning loops."""
...
def _load_data(
    ...
    # Load model-ready data (already cleaned and encoded by feature pipeline).
...
def tune_model(
    ...
    # Configure MLflow destination and experiment namespace.
    if tracking_uri:
    ...
    def objective(trial: optuna.Trial):
        # Optuna proposes one parameter set per trial.
        params = {
        ...
        # Nested run = one MLflow run per trial.
    ...
    # Retrain best model using full train split with best params.
    ...
    # Save to models/ so inference can load the artifact later.
    out = Path(model_output)
    out.parent.mkdir(parents=True, exist_ok=True)
    dump(best_model, out)
    print(f"✅ Best model saved to {out}")

    # Log final chosen model in a dedicated run.
    # Some MLflow + XGBoost versions can fail on flavor-specific logging.
    # We keep a safe fallback so tuning still succeeds end-to-end.
    with mlflow.start_run(run_name="best_xgb_model"):
        ...
        try:
            ...
            print(f"⚠️ mlflow.xgboost.log_model failed: {exc}")
            print("⚠️ Falling back to logging the saved .pkl artifact.")
            mlflow.log_artifact(str(out), artifact_path="model_artifacts")
