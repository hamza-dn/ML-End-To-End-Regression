...
TRAIN_PATH = Path("data/processed/train_final.csv")
EVAL_PATH = Path("data/processed/eval_final.csv")
...
def test_eval_works_with_saved_model(tmp_path):
    ...
    metrics = evaluate_model(
        model_path=model_path, eval_path=EVAL_PATH, sample_frac=0.01
    )
    _assert_metrics(metrics)
    print("✅ evaluate_model test passed")


# TUNE: Runs tune_model with 1 trial to keep CI/runtime lightweight.
